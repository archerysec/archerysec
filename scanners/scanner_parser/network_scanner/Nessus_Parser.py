    ...
    for data in root:
        ...
        for reportHost in data.iter("ReportHost"):
            ...
            duplicate_count = nessus_scan_results_db.objects.filter(
                username=username, scan_id=scan_id, target=target, vuln_duplicate="Yes"
            )
        ...
        ov_all_vul = nessus_scan_results_db.objects.filter(
            username=username, scan_id=scan_id, vuln_status="Open", vuln_duplicate="No"
        )
