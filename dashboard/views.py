...
def trend_update():
    ...
    for project in all_project:
        ...
        all_date_data = (ProjectDb.objects
        ...
        if len(all_month_data_display) == 0:
            add_data = MonthDb(project_id=project_id, month=current_month, high=high, medium=medium,
                               low=low)
            add_data.save()
        ...
        for data in all_month_data_display:
            ...
            match_data = MonthDb.objects.filter(project_id=project_id, month=current_month)
            if len(match_data) == 0:
                add_data = MonthDb(project_id=project_id, month=current_month, high=high,
                                   medium=medium, low=low)
                add_data.save()

            elif int(data.month) == int(current_month):
                MonthDb.objects.filter(month=current_month).update(high=high,
                                                                   medium=medium,
                                                                   low=low)
        ...
        total_close = scans_query.all_vuln_count_data(project_id=proj_id, query='Closed')
...
def dashboard(request):
    ...
    all_month_data_display = MonthDb.objects.all().values('month', 'high', 'medium', 'low').distinct()
    ...
    return render(request,
                  ...
                  {'all_project': all_project,
                   'scanners': scanners,
                   'total_count_project': ProjectDb.objects.filter().aggregate(Sum('total_vuln')),
                   'open_count_project': ProjectDb.objects.filter().aggregate(Sum('total_open')),
                   'close_count_project': ProjectDb.objects.filter().aggregate(Sum('total_close')),
                   'false_count_project': ProjectDb.objects.filter().aggregate(Sum('total_false')),
                   ...
                   'static_count_project': ProjectDb.objects.filter().aggregate(Sum('total_static')),
                   'high_count_project': ProjectDb.objects.filter().aggregate(Sum('total_high')),
                   'medium_count_project': ProjectDb.objects.filter().aggregate(Sum('total_medium')),
                   'low_count_project': ProjectDb.objects.filter().aggregate(Sum('total_low')),
                   'high_net_count_project': ProjectDb.objects.filter().aggregate(Sum('high_net')),
                   'high_web_count_project': ProjectDb.objects.filter().aggregate(Sum('high_web')),
                   ...
                   'medium_static_count_project': ProjectDb.objects.filter().aggregate(
                       Sum('medium_static')),
                   'low_net_count_project': ProjectDb.objects.filter().aggregate(Sum('low_net')),
                   'low_web_count_project': ProjectDb.objects.filter().aggregate(Sum('low_web')),
...
def proj_data(request):
    ...
    return render(request,
                  ...
                  {'project_id': uu_id,
                   ...
                   'all_static_medium': scans_query.all_static(project_id=uu_id,
                                                               query='medium'),
                   'all_static_low': scans_query.all_static(project_id=uu_id, query='low'),
                   'static_scan': static_scan,
                   'pentest': pentest,
                   'network_dat': network_dat,
                   'all_compliance_failed': scans_query.all_compliance(project_id=uu_id,
                                                                       query='failed'),
                   'all_compliance_passed': scans_query.all_compliance(project_id=uu_id,
                                                                       query='passed'),
                   'all_compliance_skipped': scans_query.all_compliance(project_id=uu_id,
                                                                        query='skipped'),
                   'total_compliance': scans_query.all_compliance(project_id=uu_id,
                                                                  query='total'),
...
def all_high_vuln(request):
    ...
    # add your scanner name here <scannername>
        ...
        web_all_high = WebScanResultsDb.objects.filter(project_id=project_id,
                                                       severity='High',
                                                       false_positive='No')
        sast_all_high = StaticScanResultsDb.objects.filter(project_id=project_id,
                                                           severity='High',
                                                           false_positive='No')
