...
def nessus_list(request):
    ...
    all_nessus_scan = (
        nessus_scan_db.objects.filter(username=username)
        .values(
            "report_name",
            "total_vuln",
            "total_high",
    ...
    return render(
        request, "networkscanners/nessus/nessusscans_list.html", {"all_nessus_scan": all_nessus_scan}
    )
...
def nessus_target_list(request):
    ...
    nessus_all_vuln = nessus_targets_db.objects.filter(
        scan_id=scan_id, username=username
    )

    return render(
        request,
        "networkscanners/nessus/nessusscan_list_vuln.html",
...
def nessus_target_data(request):
    ...
    nessus_vuln_data = nessus_scan_results_db.objects.filter(
        username=username, target=target, scan_id=scan_id
    )

    return render(
        request,
        "networkscanners/nessus/nessusscan_vuln_data.html",
...
def nessus_vuln_data(request):
    ...
    if request.method == "POST":
        ...
        nessus_targets_db.objects.filter(
            username=username, scan_id=scan_id, target=Target
        ).update(
        ...
        ov_all_vul = nessus_scan_results_db.objects.filter(
            username=username, scan_id=scan_id, false_positive="No", vuln_status="Open"
        )
        ...
        return HttpResponseRedirect(
            reverse("nessus:nessus_vuln_data")
            + "?scan_id=%s&target=%s" % (scan_id, Target)
        )
    ...
    return render(
        request,
        "networkscanners/nessus/nessusscan_vuln_data.html",
        {"nessus_vuln_data": nessus_vuln_data, "jira_url": jira_url},
    )
...
def nessus_details(request):
    ...
    nessus_vuln_details = nessus_scan_results_db.objects.filter(
        username=username, scan_id=scan_id, vuln_id=vuln_id
    )

    return render(
        request,
        "networkscanners/nessus/nessus_vuln_details.html",
...
def del_nessus(request):
    ...
    if request.method == "POST":
        ...
        for i in range(0, split_length):
            ...
            item.delete()
            item_results = nessus_scan_results_db.objects.filter(
                username=username, scan_id=scan_id
            )
            item_results.delete()
            report_result = nessus_targets_db.objects.filter(
                username=username, scan_id=scan_id
            )
...
def nessus_del_vuln(request):
    ...
    if request.method == "POST":
        ...
        for i in range(0, split_length):
            ...
            delete_vuln = nessus_scan_results_db.objects.filter(
                username=username, vuln_id=vuln_id
            )
            delete_vuln.delete()
        all_nessus_data = nessus_scan_results_db.objects.filter(
            username=username, scan_id=scan_id
        )
        ...
        all_nessus_target = nessus_scan_results_db.objects.filter(
        ...
        return HttpResponseRedirect(
            reverse("nessus:nessus_target_list") + "?scan_id=%s" % scan_id
        )
...
def export(request):
    ...
    if request.method == "POST":
        ...
        queryset = nessus_scan_results_db.objects.filter(
        ...
        if report_type.lower() == "json":
            ...
            response["Content-Disposition"] = (
                'attachment; filename="%s.json"' % "report"
            )
            return response
        if report_type.lower() == "yaml":
            response = HttpResponse(dataset.yaml, content_type="application/x-yaml")
            response["Content-Disposition"] = (
                'attachment; filename="%s.yaml"' % "report"
            )
            return response
