...
def gitlabsca_vuln_data(request):
    ...
    vuln_data_closed = gitlabsca_scan_results_db.objects.filter(username=username, scan_id=scan_id,
                                                                 message=test_name,
                                                                 vuln_status='Closed',
                                                                 false_positive='No')
    false_data = gitlabsca_scan_results_db.objects.filter(username=username, scan_id=scan_id,
                                                           message=test_name,
                                                           false_positive='Yes')
...
def export(request):
    ...
    if request.method == 'POST':
        ...
        queryset = gitlabsca_scan_results_db.objects.filter(username=username, scan_id=scan_id)
