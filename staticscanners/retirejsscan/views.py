...
def retirejsscans_list(request):
    ...
    :return:
    """
    all_retirejs_scan = retirejs_scan_db.objects.all()
...
def retirejsscan_list_vuln(request):
    ...
    retirejs_all_vuln = retirejs_scan_results_db.objects.filter(
...
def retirejsscan_vuln_data(request):
    ...
    if request.method == "POST":
        ...
        if false_positive == 'Yes':
            vuln_info = retirejs_scan_results_db.objects.filter(scan_id=scan_id, vuln_id=vuln_id)
            for vi in vuln_info:
                name = vi.test_name
    ...
    retirejs_vuln_data = retirejs_scan_results_db.objects.filter(scan_id=scan_id,
                                                                 ...
                                                                 vuln_status='Open',
                                                                 false_positive='No')
    vuln_data_closed = retirejs_scan_results_db.objects.filter(scan_id=scan_id,
                                                               test_name=test_name,
                                                               vuln_status='Closed',
                                                               false_positive='No')
    false_data = retirejs_scan_results_db.objects.filter(scan_id=scan_id,
                                                         test_name=test_name,
                                                         false_positive='Yes')
...
def retirejsscan_details(request):
    ...
    retirejs_vuln_details = retirejs_scan_results_db.objects.filter(
        scan_id=scan_id,
        vuln_id=vuln_id
    )
...
def del_retirejs_scan(request):
    ...
    if request.method == 'POST':
        ...
        for i in range(0, split_length):
            ...
            item = retirejs_scan_db.objects.filter(scan_id=scan_id)
            item.delete()
...
def retirejs_del_vuln(request):
    ...
    if request.method == 'POST':
        ...
        for i in range(0, split_length):
            ...
            delete_vuln = retirejs_scan_results_db.objects.filter(vuln_id=vuln_id)
            delete_vuln.delete()
        all_retirejs_data = retirejs_scan_results_db.objects.filter(scan_id=un_scanid)
        ...
        retirejs_scan_db.objects.filter(scan_id=un_scanid).update(
